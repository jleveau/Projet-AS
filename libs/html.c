...
char* print_balise_fonction(char* func)
{
   ...
   txt = string_concat(6,texte1,b1->texte,strdup(">"),strdup(print_function(f)),strdup("</span>"),strdup("</span>"));
   }
   else
   {
...
char* print_balise_declaration(char* func){
	balise b=print_debut_balise_id("span", "declaration");
	ajouter_attribut(b,"href","#");
	ajouter_attribut(b,"class","declaration-activable");
	
	char* texte=string_concat(6,b->texte,strdup("value=\""),strdup(func),strdup("\">"),func,strdup("</span>"));
	free(b);
	return texte;
}
