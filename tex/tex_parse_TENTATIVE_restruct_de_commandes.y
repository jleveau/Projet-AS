...
void print_fin_de_debut_balise();
void print_fin_de_debut_balise_avec_style();
%}
...
appel_commande
	: BACKSLASH commande_nom_avec_contenu{ print_fin_de_debut_balise();}OPEN_BRACE commande
	| OPEN_BRACE BACKSLASH commande_nom_avec_contenu{ print_fin_de_debut_balise();} commande
	| BACKSLASH commande_nom_avec_texte{ print_fin_de_debut_balise();} OPEN_BRACE commande
	| OPEN_BRACE BACKSLASH commande_nom_avec_texte{ print_fin_de_debut_balise();} commande
	| BACKSLASH commande_nom_avec_string{ print_fin_de_debut_balise();} OPEN_BRACE commande
	| OPEN_BRACE BACKSLASH commande_nom_avec_string{ print_fin_de_debut_balise();} commande
	| BACKSLASH commande_nom_sans_params
	| OPEN_BRACE BACKSLASH commande_nom_sans_params
	;

commande_nom_sans_params
:NEW_LINE {fprintf(f_output,"</br>");}
;

commande_nom_avec_string
...
commande_nom_avec_contenu
: TEXTTT {fprintf(f_output,"<b");} 
| TEXTIT {fprintf(f_output,"<em");} 
| UNDERLINE {fprintf(f_output,"<span style=\"text-decoration:underline;\"");} 
...
commande_nom_avec_texte
: TITLE {fprintf(f_output,"<h1>");}
| sections OPEN_BRACE texte CLOSE_BRACE {fprintf(f_output,"</section>"); /*probablement à traiter à part**************************************/}
| PARAGRAPH {fprintf(f_output,"<p style=\"");} options OPEN_BRACE{ print_fin_de_debut_balise_avec_style();} texte CLOSE_BRACE {fprintf(f_output,"</p></br>");}
...
| PARAGRAPH {fprintf(f_output,"<p style=\"");} options OPEN_BRACE{ print_fin_de_debut_balise();} texte CLOSE_BRACE {fprintf(f_output,"</p></br>");}
| NEW_LINE {fprintf(f_output,"</br>");}
| TEXTCOLOR OPEN_BRACE WORD[color] CLOSE_BRACE OPEN_BRACE {fprintf(f_output,"<span style=\"color:%s;\">", $color);} string CLOSE_BRACE  {fprintf(f_output,"</span>");}
| COLOR OPEN_BRACE WORD[color] CLOSE_BRACE {fprintf(f_output,"<span style=\"color:%s\">", $color);} texte {fprintf(f_output,"</span>");}
| BEG OPEN_BRACE paramSpecifiques[paramOpen] CLOSE_BRACE contenus fin OPEN_BRACE paramSpecifiques[paramClose] CLOSE_BRACE   {if(!strcmp($paramOpen, $paramClose)){ yyerror("open/close param différents");} /* pour begin commandes génériques ex. document etc */}  
...
sections
: SECTION {fprintf(f_output,"<section style=\"");} options {print_fin_de_debut_balise();}
        | SUB sections
        ;
